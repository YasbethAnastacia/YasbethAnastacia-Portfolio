            ...
            >
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <p
                    className={`font-semibold leading-snug transition-all duration-300 ease-out ${activaMovil ? "text-[var(--text-accent)] text-lg" : "text-[var(--text-primary)]"} ${activaActual ? "lg:text-[var(--text-accent)] lg:text-lg" : ""} group-hover:text-[var(--text-accent)]`}
                  >
                ...
                <ChevronDown
                  size={19}
                  className={`lg:hidden shrink-0 transition-all duration-500 ease-out ${activaMovil ? ` rotate-180 text-[var(--text-accent)] ` : ` rotate-0 text-[var(--text-muted)] `} group-hover:text-[var(--text-accent)]`}
                />
              </div>
            </button>
            <div
              aria-hidden={!activaMovil} inert={!activaMovil} className={`lg:hidden grid transition-[grid-template-rows,opacity] duration-500 ease-out ${activaMovil ? ` grid-rows-[1fr] opacity-100 ` : ` grid-rows-[0fr] opacity-0 `}`}
            >
