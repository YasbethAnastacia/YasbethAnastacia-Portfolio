    ...
    <div className="flex items-center gap-2">
      ...
      >
        ...
        <div
          inert={!open}
          className={
            "absolute right-0 top-[calc(100%+10px)] z-50 w-44 p-2 rounded-2xl border border-[var(--border-color)] bg-[var(--background-card)] shadow-xl transition-all duration-200 " +
        ...
        >
          {[
            ...
            <button
              ...
              className="flex w-full items-center justify-between rounded-xl p-3 text-sm text-[var(--text-primary)] hover:bg-[var(--primary-10)]"
            >
              <span>
                {flag} {label}
              </span>
  ...
  <div className={mobile ? "flex w-full flex-col items-center gap-2" : "hidden lg:flex items-center gap-6"}>
    {links.map((link) => (
      <a
        ...
        onClick={close}
        className={
          "relative rounded-xl text-sm font-medium transition-colors hover:text-[var(--text-accent)] " +
          (mobile ? "w-full px-7 py-3 text-center " : "py-2 ") +
          (active === link.href
            ? "text-[var(--text-accent)] " +
              (mobile
                ? "bg-[var(--primary-10)]"
                : "after:absolute after:bottom-0 after:left-1/2 after:h-px after:w-4 after:-translate-x-1/2 after:bg-[var(--primary-color)]")
...
const Talk = ({ label, close }) => (
  <a
    href="#contacto"
    onClick={close}
    className="inline-flex items-center justify-center gap-2 rounded-full bg-[var(--primary-color)] px-4 py-2.5 text-sm font-semibold text-[var(--on-primary)] transition-all hover:-translate-y-0.5 hover:bg-[var(--primary-hover)]"
  >
    {label}
...
export const Navbar = () => {
  ...
  return (
    <header
      className="sticky top-0 z-50 border-b border-[var(--border-color)] bg-[var(--background-card)] backdrop-blur-xl shadow-[0_8px_30px_var(--primary-10)]"
      onKeyDown={(event) => {
        if (event.key === "Escape") {
          setOpen(false);
    ...
    >
      ...
      <nav
        className="flex h-16 items-center justify-between px-5 md:px-10 lg:px-16"
      ...
      >
        <a href="#inicio" onClick={() => setOpen(false)} className="rounded-md font-semibold tracking-tight text-[var(--text-primary)]">
          <span className="text-[var(--text-accent)]">&lt;/&gt;</span> Anastacia
        </a>
        ...
        <button
          ref={toggleRef}
          type="button"
          className="nav-control w-10 px-0 lg:hidden"
      ...
      <div
        ...
        inert={!open}
        className={
          "lg:hidden grid transition-[grid-template-rows,opacity] duration-300 " + (open ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0")
        }
      >
