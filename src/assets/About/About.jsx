...
export const About = () => {
  ...
  return (
    <section
      id="sobre-mi"
      className="relative overflow-hidden px-5 md:px-10 lg:px-16 pt-12 md:pt-16 pb-16 md:pb-20 bg-[var(--background)] transition-colors duration-500"
    >
      <div
        aria-hidden="true"
        className="pointer-events-none absolute right-[8%] top-10 w-40 h-40 rounded-full bg-[var(--primary-color)] opacity-15 blur-3xl "
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[0.85fr_1.15fr] gap-12 lg:gap-16 items-center">
        <div
          ref={photoRef}
          className={`relative flex justify-center transition-all duration-700 ease-out ${photoVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-8"}`}
        >
          <div className="relative flex items-center justify-center w-[320px] h-[360px] md:w-[390px] md:h-[430px]">
            ...
            <div className="group relative z-10 w-[260px] h-[260px] md:w-[320px] md:h-[320px] rounded-full overflow-hidden bg-[var(--background-soft)] shadow-xl ring-8 ring-[var(--background-card)] transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl">
              <img
                src="/HERO/mi.jpeg"
                alt={t("Retrato de Yasbeth Anastacia Vázquez Hernández")}
            ...
            <div className="absolute z-20 bottom-0 left-1/2 -translate-x-1/2 flex items-center gap-3 px-5 py-3 rounded-2xl bg-[var(--background-card)] backdrop-blur-md shadow-lg border border-[var(--border-color)] whitespace-nowrap transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ">
              ...
              <div>
                <p className="text-[11px] text-[var(--text-muted)] font-medium">{t("En constante aprendizaje")}</p>
                <p className="text-sm text-[var(--text-primary)] font-semibold">{t("Frontend Developer")}</p>
              </div>
            </div>
          </div>
        </div>
        <div
          ref={textRef}
          className={`flex flex-col gap-6 transition-all duration-700 ease-out ${textVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}
        >
          <div className="flex flex-col gap-2">
            ...
            <h2 className="text-3xl md:text-4xl lg:text-[42px] font-semibold text-[var(--text-primary)] leading-tight transition-colors duration-500">
              {t("¡Hola! Soy")} <span className="text-[var(--text-accent)]">Anastacia</span>
            </h2>
          </div>
          <div className="flex flex-col gap-4 text-[var(--text-secondary)] text-base md:text-lg leading-relaxed max-w-2xl transition-colors duration-500">
            ...
            <p>
              ...
              <span className="font-semibold text-[var(--text-accent)]">{t("Frontend Developer")}</span>
